...
export class Symbol extends AlgorithmBase {
  ...
  private static getIndex(index, additional, max): number {
    if (additional < 0) {
      return (index + additional + (-1 * additional * max)) % max;
    } else {
      return (index + additional) % max;
    }
  }
    ...
    for (let i = 0; i < this.settings.code.length; i++) {
      for (let j = 0; j < this.settings.code[i].length; j++) {
        if (this.settings.code[i][j].indexOf(part.before.toUpperCase()) !== -1) {
          const ii = Symbol.getIndex(i, additionalRowIndex, this.settings.code.length);
          const jj = Symbol.getIndex(j, additionalColIndex, this.settings.code[0].length);
